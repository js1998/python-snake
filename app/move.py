...
def find_next_direction(occupied, body_pos, width, height, dest):
    ...
    while len(open_list) != 0:
        ...
        if is_possible_move("up", body_pos, occupied, height, width) and not point_in_list(closed_list, up_point)[0]:
            open_list = find_path(open_list, curr_node, up_point, dest)

        down_point = {"x": head_pos["x"], "y": head_pos["y"] + 1}
        if is_possible_move("down", body_pos, occupied, height, width) and not point_in_list(closed_list, down_point)[0]:
            open_list = find_path(open_list, curr_node, down_point, dest)

        right_point = {"x": head_pos["x"] + 1, "y": head_pos["y"]}
        if is_possible_move("right", body_pos, occupied, height, width) and not point_in_list(closed_list, right_point)[0]:
            open_list = find_path(open_list, curr_node, right_point, dest)

        left_point = {"x": head_pos["x"] - 1, "y": head_pos["y"]}
        if is_possible_move("left", body_pos, occupied, height, width) and not point_in_list(closed_list, left_point)[0]:
